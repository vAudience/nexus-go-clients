/*
vAudience AI File Manager API

API for the vAudience AI File Manager

API version: 0.22.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package filemanager

import (
	"encoding/json"
)

// checks if the CollectionFileListResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CollectionFileListResponse{}

// CollectionFileListResponse struct for CollectionFileListResponse
type CollectionFileListResponse struct {
	Collection *CollectionResponse `json:"collection,omitempty"`
	Files []CollectionFileResponse `json:"files,omitempty"`
	Limit *int32 `json:"limit,omitempty"`
	Offset *int32 `json:"offset,omitempty"`
	Total *int64 `json:"total,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _CollectionFileListResponse CollectionFileListResponse

// NewCollectionFileListResponse instantiates a new CollectionFileListResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCollectionFileListResponse() *CollectionFileListResponse {
	this := CollectionFileListResponse{}
	return &this
}

// NewCollectionFileListResponseWithDefaults instantiates a new CollectionFileListResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCollectionFileListResponseWithDefaults() *CollectionFileListResponse {
	this := CollectionFileListResponse{}
	return &this
}

// GetCollection returns the Collection field value if set, zero value otherwise.
func (o *CollectionFileListResponse) GetCollection() CollectionResponse {
	if o == nil || IsNil(o.Collection) {
		var ret CollectionResponse
		return ret
	}
	return *o.Collection
}

// GetCollectionOk returns a tuple with the Collection field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionFileListResponse) GetCollectionOk() (*CollectionResponse, bool) {
	if o == nil || IsNil(o.Collection) {
		return nil, false
	}
	return o.Collection, true
}

// HasCollection returns a boolean if a field has been set.
func (o *CollectionFileListResponse) HasCollection() bool {
	if o != nil && !IsNil(o.Collection) {
		return true
	}

	return false
}

// SetCollection gets a reference to the given CollectionResponse and assigns it to the Collection field.
func (o *CollectionFileListResponse) SetCollection(v CollectionResponse) {
	o.Collection = &v
}

// GetFiles returns the Files field value if set, zero value otherwise.
func (o *CollectionFileListResponse) GetFiles() []CollectionFileResponse {
	if o == nil || IsNil(o.Files) {
		var ret []CollectionFileResponse
		return ret
	}
	return o.Files
}

// GetFilesOk returns a tuple with the Files field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionFileListResponse) GetFilesOk() ([]CollectionFileResponse, bool) {
	if o == nil || IsNil(o.Files) {
		return nil, false
	}
	return o.Files, true
}

// HasFiles returns a boolean if a field has been set.
func (o *CollectionFileListResponse) HasFiles() bool {
	if o != nil && !IsNil(o.Files) {
		return true
	}

	return false
}

// SetFiles gets a reference to the given []CollectionFileResponse and assigns it to the Files field.
func (o *CollectionFileListResponse) SetFiles(v []CollectionFileResponse) {
	o.Files = v
}

// GetLimit returns the Limit field value if set, zero value otherwise.
func (o *CollectionFileListResponse) GetLimit() int32 {
	if o == nil || IsNil(o.Limit) {
		var ret int32
		return ret
	}
	return *o.Limit
}

// GetLimitOk returns a tuple with the Limit field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionFileListResponse) GetLimitOk() (*int32, bool) {
	if o == nil || IsNil(o.Limit) {
		return nil, false
	}
	return o.Limit, true
}

// HasLimit returns a boolean if a field has been set.
func (o *CollectionFileListResponse) HasLimit() bool {
	if o != nil && !IsNil(o.Limit) {
		return true
	}

	return false
}

// SetLimit gets a reference to the given int32 and assigns it to the Limit field.
func (o *CollectionFileListResponse) SetLimit(v int32) {
	o.Limit = &v
}

// GetOffset returns the Offset field value if set, zero value otherwise.
func (o *CollectionFileListResponse) GetOffset() int32 {
	if o == nil || IsNil(o.Offset) {
		var ret int32
		return ret
	}
	return *o.Offset
}

// GetOffsetOk returns a tuple with the Offset field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionFileListResponse) GetOffsetOk() (*int32, bool) {
	if o == nil || IsNil(o.Offset) {
		return nil, false
	}
	return o.Offset, true
}

// HasOffset returns a boolean if a field has been set.
func (o *CollectionFileListResponse) HasOffset() bool {
	if o != nil && !IsNil(o.Offset) {
		return true
	}

	return false
}

// SetOffset gets a reference to the given int32 and assigns it to the Offset field.
func (o *CollectionFileListResponse) SetOffset(v int32) {
	o.Offset = &v
}

// GetTotal returns the Total field value if set, zero value otherwise.
func (o *CollectionFileListResponse) GetTotal() int64 {
	if o == nil || IsNil(o.Total) {
		var ret int64
		return ret
	}
	return *o.Total
}

// GetTotalOk returns a tuple with the Total field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CollectionFileListResponse) GetTotalOk() (*int64, bool) {
	if o == nil || IsNil(o.Total) {
		return nil, false
	}
	return o.Total, true
}

// HasTotal returns a boolean if a field has been set.
func (o *CollectionFileListResponse) HasTotal() bool {
	if o != nil && !IsNil(o.Total) {
		return true
	}

	return false
}

// SetTotal gets a reference to the given int64 and assigns it to the Total field.
func (o *CollectionFileListResponse) SetTotal(v int64) {
	o.Total = &v
}

func (o CollectionFileListResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CollectionFileListResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Collection) {
		toSerialize["collection"] = o.Collection
	}
	if !IsNil(o.Files) {
		toSerialize["files"] = o.Files
	}
	if !IsNil(o.Limit) {
		toSerialize["limit"] = o.Limit
	}
	if !IsNil(o.Offset) {
		toSerialize["offset"] = o.Offset
	}
	if !IsNil(o.Total) {
		toSerialize["total"] = o.Total
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *CollectionFileListResponse) UnmarshalJSON(data []byte) (err error) {
	varCollectionFileListResponse := _CollectionFileListResponse{}

	err = json.Unmarshal(data, &varCollectionFileListResponse)

	if err != nil {
		return err
	}

	*o = CollectionFileListResponse(varCollectionFileListResponse)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "collection")
		delete(additionalProperties, "files")
		delete(additionalProperties, "limit")
		delete(additionalProperties, "offset")
		delete(additionalProperties, "total")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableCollectionFileListResponse struct {
	value *CollectionFileListResponse
	isSet bool
}

func (v NullableCollectionFileListResponse) Get() *CollectionFileListResponse {
	return v.value
}

func (v *NullableCollectionFileListResponse) Set(val *CollectionFileListResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableCollectionFileListResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableCollectionFileListResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCollectionFileListResponse(val *CollectionFileListResponse) *NullableCollectionFileListResponse {
	return &NullableCollectionFileListResponse{value: val, isSet: true}
}

func (v NullableCollectionFileListResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCollectionFileListResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


