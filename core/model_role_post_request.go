...
type RolePostRequest struct {
	...
	// Permissions the role grants. An organization may grant only the permissions its owner holds, plus the org-admin and org-member struct-access levels; anything else is refused. Duplicates are removed, so the stored array can differ from the one posted.
