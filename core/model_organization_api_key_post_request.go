...
type OrganizationApiKeyPostRequest struct {
	...
	// Global roles to attach. Requires the manage-api-key-roles:organizations permission (vaudience staff only) whenever the field is present, an empty array included. Product roles are never assignable: the active subscription's role is attached automatically. Omit the field entirely to create a key without extra roles.
