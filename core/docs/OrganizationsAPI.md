...
[**BulkInvite**](OrganizationsAPI.md#BulkInvite) | **Post** /v1/organizations/{id}/invites/bulk | Bulk invite users to an Organization
...
> OrganizationBulkInviteResponse BulkInvite(ctx, id).OrganizationBulkInvite(organizationBulkInvite).Execute()
...
func main() {
	id := "id_example" // string | id of the organization
	organizationBulkInvite := *openapiclient.NewOrganizationBulkInviteRequest([]openapiclient.BulkInviteEntry{*openapiclient.NewBulkInviteEntry("Email_example")}) // OrganizationBulkInviteRequest | organization bulk invite object

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.OrganizationsAPI.BulkInvite(context.Background(), id).OrganizationBulkInvite(organizationBulkInvite).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `OrganizationsAPI.BulkInvite``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `BulkInvite`: OrganizationBulkInviteResponse
	fmt.Fprintf(os.Stdout, "Response from `OrganizationsAPI.BulkInvite`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiBulkInviteRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------

 **organizationBulkInvite** | [**OrganizationBulkInviteRequest**](OrganizationBulkInviteRequest.md) | organization bulk invite object | 
