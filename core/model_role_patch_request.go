...
type RolePatchRequest struct {
	...
	// Replaces the role's permissions, under the same allow-list and de-duplication as RolePostRequest. Omit the field or send null to leave the stored permissions untouched; an empty array is refused. (Maintainers: see the \"Permissions validate-tag contract\" comment in organizations_association_permissions.go before changing the validate tag.)
