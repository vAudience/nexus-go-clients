...
type ApiExportPromptsRequest struct {
	ctx context.Context
	ApiService *PromptsAPIService
	orgId string
}

func (r ApiExportPromptsRequest) Execute() (*http.Response, error) {
	return r.ApiService.ExportPromptsExecute(r)
}

/*
ExportPrompts Export prompts as CSV

Download a CSV file containing all prompt templates in the organization for audit/compliance purposes.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param orgId Organization ID
 @return ApiExportPromptsRequest
*/
func (a *PromptsAPIService) ExportPrompts(ctx context.Context, orgId string) ApiExportPromptsRequest {
	return ApiExportPromptsRequest{
		ApiService: a,
		ctx: ctx,
		orgId: orgId,
...
// Execute executes the request
func (a *PromptsAPIService) ExportPromptsExecute(r ApiExportPromptsRequest) (*http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "PromptsAPIService.ExportPrompts")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v1/organizations/{org_id}/prompts/export"
	localVarPath = strings.Replace(localVarPath, "{"+"org_id"+"}", url.PathEscape(parameterValueToString(r.orgId, "orgId")), -1)
